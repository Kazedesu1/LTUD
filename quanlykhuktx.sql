...
-- Thêm cột so_tien vào bảng ChiTietHoaDon
ALTER TABLE ChiTietHoaDon
ADD so_tien DECIMAL(10,2) NOT NULL;
...
BEGIN
    SET NOCOUNT ON;
    SELECT id.ma_chi_tiet, 
           s.ten_dich_vu, 
           id.so_tien
    FROM ChiTietHoaDon id
    JOIN DichVu s ON id.ma_dich_vu = s.ma_dich_vu
    WHERE id.ma_hoa_don = @ma_hoa_don;
END;
GO
...
BEGIN
    ...
    SELECT @ma_sinh_vien = ma_sinh_vien FROM inserted;

    -- Gọi thủ tục tạo hóa đơn
    EXEC TaoHoaDon @ma_sinh_vien;
END;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM HoaDon WHERE ma_hoa_don = @ma_hoa_don AND trang_thai_thanh_toan = N'Đã thanh toán')
    BEGIN
    ...
    DELETE FROM ChiTietHoaDon WHERE ma_hoa_don = @ma_hoa_don;

    -- Xóa hóa đơn
    DELETE FROM HoaDon WHERE ma_hoa_don = @ma_hoa_don;
...
BEGIN
    SET NOCOUNT ON;
    SELECT SUM(tong_so_tien) AS Tong_Doanh_Thu
    FROM HoaDon
    WHERE trang_thai_thanh_toan = N'Đã thanh toán';
END;
...
BEGIN
    SET NOCOUNT ON;
    SELECT trang_thai, COUNT(*) AS tong_so_khieu_nai
    FROM KhieuNai
    GROUP BY trang_thai;
END;

-- Gọi thủ tục XemDanhSachKhieuNai để xem tất cả các khiếu nại
EXEC XemDanhSachKhieuNai;

-- Gửi một khiếu nại mới
EXEC GuiKhieuNai @ma_sinh_vien = 1, @noi_dung_khieu_nai = N'Phòng không có Wi-Fi';

-- Xem tất cả các khiếu nại
EXEC XemDanhSachKhieuNai;
...
CREATE View XemDanhSachSinhVien
AS
    SELECT * FROM SinhVien

CREATE VIEW XemDanhSachPhong AS
SELECT *, (suc_chua - so_nguoi_hien_tai) AS sochotrong
FROM PHONG;

CREATE view XemThongKeDichVu
AS
	SELECT
		dv.ten_dich_vu,
		COUNT(dkdv.ma_sinh_vien) AS SoLuongSinhVienDangKy
	FROM DangKyDichVu dkdv
	JOIN DichVu dv ON dkdv.ma_dich_vu = dv.ma_dich_vu
	GROUP BY dv.ten_dich_vu;



CREATE view XemThongKeChuaThanhToanHoaDon
AS
	SELECT DISTINCT sv.ma_sinh_vien, sv.ho_ten, sv.ma_phong, sv.trang_thai
	FROM HoaDon hd
	JOIN SinhVien sv ON hd.ma_sinh_vien = sv.ma_sinh_vien
	WHERE hd.trang_thai_thanh_toan = N'Chưa thanh toán';


Alter view XemThongKeKhieuNaiChuaXuLy
AS

	SELECT kn.ma_khieu_nai, sv.ho_ten, kn.trang_thai, kn.noi_dung_khieu_nai
	FROM KhieuNai kn
	JOIN SinhVien sv ON kn.ma_sinh_vien = sv.ma_sinh_vien
	WHERE kn.trang_thai IN (N'Chưa giải quyết', N'Đang xử lý');
select * from XemDanhSachSinhVien
select * from KhieuNai